...
# --- 🧠 元趨勢戰法 (Meta-Trend) [V90.1 諸神黃昏最終整合版] ---
# ==============================================================================
# Titan SOP V90.1 - 諸神黃昏 (Ragnarök)
# [CORE FEATURES]:
# 1. 幾何引擎 (compute_7d_geometry): 支援 Slot 6.1 與 6.4 共用運算
# 2. 參謀本部 (TitanAgentCouncil): 五大角鬥士 + 防偷懶協議
# 3. 戰略工廠 (Slot 6.2): 智能連結 + 檔案情報 + 20條原則
# 4. 索敵即獵殺 (Slot 6.4): 掃描後直接生成戰略 Prompt
# ==============================================================================

# [V90.1 核心] 幾何運算引擎 (獨立函式，供全模組調用)
def compute_7d_geometry(ticker):
    """
    計算 7 維度幾何數據 (35Y, 10Y, 5Y, 3Y, 1Y, 6M, 3M)
    返回: 包含 angle (角度) 與 r2 (穩定度) 的字典
    """
    try:
        # 智慧代碼處理
        ...
        # 轉月K (以月為單位計算幾何更穩定)
        df_m = df.resample('ME').last() # pandas 2.0+ 使用 'ME'
        if df_m.empty: df_m = df.resample('M').last() # 舊版兼容
...
class TitanAgentCouncil:
    ...
    def __init__(self, api_key=None):
        ...
        self.model = None
        # 若有 API Key 則初始化模型，否則僅用於生成 Prompt
        if api_key:
            try:
                genai.configure(api_key=api_key)
...
* **論點**：別跟我談 PE，看 2033 年後的下個景氣循環 TAM (潛在市場)。
...
def render_meta_trend():
    ...
    st.caption("全歷史幾何 × 五大角鬥士 × 全境獵殺 | 核心目標：鎖定 2033 年後百倍股")
    st.markdown("---")
    ...
    with t2:
        ...
        with col_params:
            ...
            if uploaded_intel_file:
                intel_status = f"\n[系統備註]: 使用者已上傳檔案 {uploaded_intel_file.name}"
                st.success("✅ 檔案已接收 (將併入 Prompt)")
            
            final_intel = intel_text_manual + intel_status
        ...
        with col_output:
            ...
            if 'generated_prompt' in st.session_state:
                ...
                st.download_button(
                    "💾 下載戰略提示詞 (.txt)",
                    prompt_text,
                    file_name=f"TITAN_PROMPT_{st.session_state.meta_target}.txt",
                    mime="text/plain",
                    use_container_width=True
                )
    ...
    with t4:
        ...
        if st.button(f"🚀 掃描 {theater}", use_container_width=True):
            ...
            for i, t in enumerate(targets):
                ...
                if g:
                    # 簡單濾網: 3M > 20度 或 加速度 > 10
                    acc = g['3M']['angle'] - g['1Y']['angle']
                    if g['3M']['angle'] > 20 or acc > 10:
                        results.append({
                            'Ticker': t, 
                            '3M_Angle': g['3M']['angle'],
                            'Acceleration': acc,
            ...
            if target_t:
                ...
                with st.expander(f"🔥 生成 {target_t} 戰略提示詞", expanded=True):
                    ...
                    if st.button("生成提示詞", key=f"btn_{target_t}"):
                        ...
                        st.text_area("Prompt", p, height=200)
                        st.download_button("下載 .txt", p, file_name=f"HUNT_{target_t}.txt")
