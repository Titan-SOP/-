...
# ==========================================
# Titan SOP V90.1 - 諸神黃昏 (Ragnarök)
# [V90.1 Patch]:
# 1. [Structure] Tab 2 升級為「戰略工廠」：整合情報、原則與輸出。
# 2. [Methodology] 植入「20 條第一性原則」：在 Tab 2 提供多選清單，強制 AI 遵守。
# 3. [Structure] Tab 4 升級為「全境獵殺」：掃描後可直接對目標進行深度分析。
# 4. [Intelligence] 強化情報注入：Tab 2 支援 PDF/Word/Excel 文件上傳。
# 5. [UX] 新增智能快捷連結：在 Tab 2 提供 9 大常用財經網站連結。
# ==========================================
...
import plotly.graph_objects as go
import google.generativeai as genai
from config import WAR_THEATATERS # [V89.1 新增] 全境獵殺戰區清單
import io
import pdfplumber
import docx
import openpyxl
...
class TitanAgentCouncil:
    ...
    def __init__(self, api_key=None):
        ...
        if api_key:
            try:
                genai.configure(api_key=api_key)
                # V90.0: 優先使用最新的 Gemini 1.5 Flash
...
**【⚠️ 最高禁令 (Anti-Laziness Protocol)】**
1.  **禁止省略**: 絕對不准使用 `...` 或 `pass` 來省略任何角色的發言。
...
5.  **互動續寫**: 後面的角色必須針對前面角色的發言進行反駁或支持，形成連貫的辯論串。
...
# ==========================================
# [SLOT-6.6] 主渲染函式 (Main Rendering Function)
# ==========================================
@st.fragment
def render_meta_trend():
    ...
    """
    # 返回首頁按鈕
    if st.button("🏠 返回戰情總部", type="secondary"):
        st.session_state.page = 'home'
        st.rerun()
    ...
    with tab1:
        ...
        if ticker in st.session_state.get('daily_price_data', {}):
            ...
            if df_daily is not None and not df_daily.empty:
                df_chart = df_daily.copy().reset_index()
                ...
                slope, intercept, r_value, p_value, std_err = linregress(df_chart['Days'].values, log_prices)
                ...
                col_stat1.metric("全歷史 R²", f"{r_value**2:.4f}")
                col_stat2.metric("當前價格", f"${current_price:.2f}")
                col_stat3.metric("趨勢線乖離", f"{deviation:+.1f}%", delta_color="normal" if abs(deviation) < 20 else "inverse")
                
                price_line = alt.Chart(df_chart).mark_line(color='#00FF00', strokeWidth=2).encode(
                    x=alt.X('Date:T', title='時間', axis=alt.Axis(format='%Y')),
                    y=alt.Y('Close:Q', title='收盤價 (對數座標)', scale=alt.Scale(type='log'), axis=alt.Axis(tickCount=10)),
                    tooltip=[alt.Tooltip('Date:T', title='日期', format='%Y-%m-%d'), alt.Tooltip('Close:Q', title='收盤價', format=',.2f'), alt.Tooltip('Trendline:Q', title='趨勢線', format=',.2f')]
                ).properties(height=500, title=f'{ticker} - 全歷史對數線性回歸分析 (1990-Now)')
                
                trend_line = alt.Chart(df_chart).mark_line(color='#4169E1', strokeWidth=2, strokeDash=[5, 5]).encode(x='Date:T', y=alt.Y('Trendline:Q', scale=alt.Scale(type='log')))
                
                final_chart = (price_line + trend_line).configure_axis(gridColor='#333333', domainColor='#666666').configure_view(strokeWidth=0)
                st.altair_chart(final_chart, use_container_width=True)
            else:
    ...
    # ==========================================
    # [TAB 2] Slot 6.2 戰略工廠 - V90.1 升級
    # ==========================================
    with tab2:
        st.header("🏭 戰略工廠 (Strategy Factory)")
        st.caption("整合情報、原則與 AI 辯論，生成最終戰略提示詞")
        ...
        with col_params:
            ...
            with st.expander("🔗 智能快捷鏈接 (Quick Links)", expanded=True):
                ticker_clean = ticker.replace('.TW', '').replace('.TWO', '')
                st.page_link(f"https://www.tradingview.com/symbols/{ticker_clean}/", label="TradingView", icon="📈")
                st.page_link(f"https://finviz.com/quote.ashx?t={ticker_clean}", label="Finviz", icon="🗺️")
                st.page_link(f"https://finance.yahoo.com/quote/{ticker_clean}", label="Yahoo Finance", icon="📊")
                st.page_link(f"https://stockcharts.com/h-sc/ui?s={ticker_clean}", label="StockCharts", icon="📉")
                st.page_link(f"https://news.cnyes.com/search?keyword={ticker_clean}", label="鉅亨網", icon="🇹🇼")
                st.page_link(f"https://goodinfo.tw/tw/StockDetail.asp?STOCK_ID={ticker_clean}", label="Goodinfo", icon="🇹🇼")
                st.page_link(f"https://mops.twse.com.tw/mops/web/t05st01", label="公開資訊觀測站", icon="🇹🇼")
                st.page_link(f"https://www.alphamemo.ai/free-transcripts", label="AlphaMemo (法說會)", icon="🎙️")
                st.page_link(f"https://www.sec.gov/edgar/searchedgar/companysearch", label="SEC Edgar (財報)", icon="🇺🇸")

            with st.expander("🕵️ 情報注入 (Intel Injection)", expanded=True):
                intel_text_manual = st.text_area("貼上法說會摘要、財報數據或新聞", height=150)
                uploaded_intel_file = st.file_uploader("或上傳情報文件 (PDF/Word/Excel)", type=['pdf', 'docx', 'xlsx'])
                
                intel_text_file = ""
                if uploaded_intel_file:
                    try:
                        if uploaded_intel_file.name.endswith('.pdf'):
                            with pdfplumber.open(uploaded_intel_file) as pdf:
                                intel_text_file = "\n".join(page.extract_text() for page in pdf.pages)
                        elif uploaded_intel_file.name.endswith('.docx'):
                            doc = docx.Document(uploaded_intel_file)
                            intel_text_file = "\n".join(para.text for para in doc.paragraphs)
                        elif uploaded_intel_file.name.endswith('.xlsx'):
                            wb = openpyxl.load_workbook(uploaded_intel_file)
                            sheet = wb.active
                            intel_text_file = "\n".join(str(cell.value) for row in sheet.iter_rows() for cell in row if cell.value)
                        st.success(f"✅ 已讀取 {uploaded_intel_file.name}")
                    except Exception as e:
                        st.error(f"文件讀取失敗: {e}")
                
                final_intel_text = intel_text_manual + "\n\n" + intel_text_file

            with st.expander("✍️ 統帥第一性原則 (Commander's Override)", expanded=True):
                principles = [
                ...
                selected_principles = st.multiselect("選擇要強制 AI 遵守的原則 (最多 5 項)", options=principles, max_selections=5)
                commander_note_final = "\n".join(f"- {p}" for p in selected_principles)
        ...
        with col_output:
            ...
            if st.button("🔥 生成史詩級戰略提示詞", type="primary", use_container_width=True):
                ...
                battle_prompt = council.generate_battle_prompt(
                    ticker, current_price, geo, rating, final_intel_text, commander_note_final
                )
                st.session_state.battle_prompt = battle_prompt
            
            if 'battle_prompt' in st.session_state:
                st.text_area("📋 複製此提示詞 (Ctrl+A, Ctrl+C)", value=st.session_state.battle_prompt, height=400)
                st.download_button(
                    "💾 下載戰略提示詞 (.txt)",
    ...
    # ==========================================
    with tab3:
        st.subheader("📝 條件式獵殺清單 (Watchlist)")
        ...
        if any(hr in rating[0] for hr in high_ratings):
            ...
            if st.button(f"✅ 存入獵殺清單", type="primary"):
                if 'watchlist' not in st.session_state:
                    st.session_state.watchlist = pd.DataFrame(columns=["代號", "加入時評級", "加入日期"])
                
                if ticker not in st.session_state.watchlist['代號'].values:
                    new_entry = pd.DataFrame([{"代號": ticker, "加入時評級": rating[0], "加入日期": datetime.now().strftime('%Y-%m-%d')}])
                    st.session_state.watchlist = pd.concat([st.session_state.watchlist, new_entry], ignore_index=True)
                    st.toast(f"🎯 {ticker} 已加入獵殺清單！", icon="✅")
                else:
    ...
    with tab4:
        ...
        with st.expander("🎯 獵殺控制台 (Mission Control)", expanded=True):
            ...
            if st.button("🚀 啟動全境掃描", type="primary", use_container_width=True):
                ...
                for i, t in enumerate(tickers_to_scan):
                    ...
                    if geo_data:
                        rating_data = titan_rating_system(geo_data)
                        hunt_results.append({
                            "代號": t,
                            "評級": rating_data[0],
                            "名稱": rating_data[1],
                            ...
                            "Phoenix": "🔥" if geo_data['phoenix_signal'] else "❄️"
                        })
                
                progress_bar.empty()
                st.session_state[f'hunt_results_{selected_theater}'] = pd.DataFrame(hunt_results)
                st.success(f"✅ {selected_theater} 戰區掃描完成！")
        ...
        if f'hunt_results_{selected_theater}' in st.session_state:
            ...
            if not results_df.empty:
                ...
                target_options = results_df['代號'].tolist()
                selected_target = st.selectbox("選擇目標進行深度分析", options=[""] + target_options)
                
                if selected_target:
                    st.subheader(f"🔬 深度分析: {selected_target}")
                    with st.spinner("正在為選定目標生成分析參數..."):
                        target_geo = compute_7d_geometry(selected_target)
                        target_rating = titan_rating_system(target_geo)
                        target_price = 0.0
                        if selected_target in st.session_state.get('daily_price_data', {}):
                            df_daily = st.session_state.daily_price_data[selected_target]
                            if df_daily is not None and not df_daily.empty:
                                target_price = df_daily['Close'].iloc[-1]
                    
                    st.info(f"目標: {selected_target} | 評級: {target_rating[0]} | 現價: ${target_price:.2f}")
                    
                    # 嵌入迷你版戰略工廠
                    mini_intel = st.text_area("注入此目標的特定情報", key=f"intel_{selected_target}", height=100)
                    mini_principles_list = [
                        "[成長] 萊特定律檢視：產量翻倍，成本是否下降 15%？", "[生存] 燒錢率生存測試：若 18 個月融不到資，會死嗎？",
                        "[泡沫] 均值回歸引力：利潤率若回歸平均，股價會腰斬嗎？", "[終極] 不可替代性：若公司明天消失，世界有差嗎？"
                    ]
                    mini_principles = st.multiselect("選擇核心原則", options=mini_principles_list, key=f"prin_{selected_target}")
                    mini_commander_note = "\n".join(f"- {p}" for p in mini_principles)
                    
                    if st.button(f"生成 {selected_target} 戰略提示詞", key=f"gen_{selected_target}"):
                        council = TitanAgentCouncil()
                        prompt = council.generate_battle_prompt(
                            selected_target, target_price, target_geo, target_rating, mini_intel, mini_commander_note
                        )
                        st.text_area("複製提示詞", value=prompt, height=300, key=f"prompt_{selected_target}")
...
def render_home():
    ...
    with col6:
        if st.button("🧠 元趨勢戰法\n(Meta-Trend)"):
            st.session_state.page = 'meta_trend'
            st.rerun()
...
with st.sidebar:
    ...
    if f_cb_list:
        try:
            ...
            if underlying_price_col: rename_map[underlying_price_col] = 'underlying_price'
            ...
            if missing_cols:
                ...
                if 'avg_volume' not in df.columns:
                    ...
                    if vol_col: df.rename(columns={vol_col: 'avg_volume'}, inplace=True)
                    else: df['avg_volume'] = 100
...
if st.session_state.page == 'home':
    ...
    render_radar()
elif st.session_state.page == 'sniper':
    render_sniper()
