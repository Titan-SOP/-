...
"""
    ...
    def run_debate(self, ticker, price, geo_data, rating_info, intel_text="", commander_note="", selected_principles=None):
        """
        執行 AI 辯論並返回結果
        """
        if not self.model:
...
def render_meta_trend():
    ...
    st.caption("全歷史幾何 × 五大角鬥士 × 20 條第一性原則 × 全境獵殺 | 核心目標：鎖定 2033 年百倍股")
    st.markdown("---")
    ...
    with tab2:
        ...
        with col_params:
            ...
            with st.expander("🔗 智能快捷連結 (9 個必備資源)", expanded=True):
                ...
                if ticker.endswith('.TW') or ticker.endswith('.TWO'):
                    ...
                    st.markdown(f"""
                    3. **[Yahoo 台股](https://tw.stock.yahoo.com/quote/{ticker_clean})** - 台股即時報價與新聞
                    """)
                else:
                    st.markdown(f"""
                ...
                if not (ticker.endswith('.TW') or ticker.endswith('.TWO')):
                    ...
                    st.markdown(f"""
                    9. **[證券櫃檯買賣中心](https://www.tpex.org.tw/web/stock/aftertrading/otc_quotes_no1430/stk_wn1430.php?l=zh-tw)** - 上櫃股票資訊
                    """)
            ...
            with st.expander("🕵️ 多源情報注入 (Intel Upload)", expanded=True):
                ...
                if uploaded_files_factory:
                    for file in uploaded_files_factory:
                        uploaded_content_factory += f"\n[上傳檔案: {file.name}]\n"
                        st.caption(f"✅ 已上傳: {file.name}")
            ...
            with st.expander("🎯 統帥第一性原則 (20 條完整清單)", expanded=True):
                ...
                selected_principles_factory = st.multiselect(
                    "選擇第一性原則 (可多選)",
                    options=first_principles,
                    default=[],
                    key="principles_factory"
                )
                
                st.caption(f"✅ 已選擇 {len(selected_principles_factory)} 條原則")
            ...
            with st.expander("✍️ 統帥自由筆記 (Commander's Note)", expanded=False):
                ...
                commander_note_factory = st.text_area(
                    "統帥筆記",
                    height=120,
                    placeholder="例如：重點關注現金流與毛利率趨勢，忽略短期股價波動...",
                    key="commander_note_factory"
                )
        ...
        with col_output:
            ...
            # 獲取當前價格
            current_price_factory = 0.0
            if ticker in st.session_state.get('daily_price_data', {}):
                df_daily = st.session_state.daily_price_data[ticker]
                if df_daily is not None and not df_daily.empty:
                    current_price_factory = df_daily['Close'].iloc[-1]
            
            # 顯示當前標的資訊
            st.info(f"""
            **當前標的**: {ticker}  
            **現價**: ${current_price_factory:.2f}  
            **信評**: {rating[0]} - {rating[1]}  
            **已選原則**: {len(selected_principles_factory)} 條
            """)
            ...
            if st.button("🚀 生成戰略提示詞", type="primary", use_container_width=True):
                ...
                if uploaded_content_factory:
                    combined_intel_factory += uploaded_content_factory
                
                # 生成提示詞
                council_factory = TitanAgentCouncil()  # 不需要 API Key
                battle_prompt_factory = council_factory.generate_battle_prompt(
                    ticker, 
                    current_price_factory, 
                    geo, 
                    rating, 
                    combined_intel_factory, 
                    commander_note_factory,
                    selected_principles_factory
                )
                ...
                st.text_area(
                    ...
                    value=battle_prompt_factory,
                    height=400,
                    key="prompt_output_factory"
                )
    ...
    with tab4:
            ...
            if st.button("🚀 啟動全境掃描", type="primary", use_container_width=True, key="start_hunt_btn"):
                if not selected_theater:
                    ...
                    for i, t in enumerate(tickers_to_scan):
                        geo_data_hunt = compute_7d_geometry(t)
                        progress_bar.progress((i + 1) / total_tickers, text=f"掃描進度: {t} ({i+1}/{total_tickers})")
                        
                        if geo_data_hunt:
                            # 獲取現價
                            current_price_hunt = 0.0
                            if t in st.session_state.get('daily_price_data', {}) and not st.session_state.daily_price_data[t].empty:
                                current_price_hunt = st.session_state.daily_price_data[t]['Close'].iloc[-1]

                            # 濾網條件判斷
                            match_type = None
                            # 模式 A: Phoenix
                            if geo_data_hunt['10Y']['angle'] < 10 and geo_data_hunt['3M']['angle'] > 45:
                                match_type = "🔥 Phoenix"
                            # 模式 B: Awakening
                            elif abs(geo_data_hunt['35Y']['angle']) < 15 and geo_data_hunt['acceleration'] > 20:
                                match_type = "🦁 Awakening"
                            # 模式 C: Rocket
                            elif geo_data_hunt['3M']['angle'] > 60:
                                match_type = "🚀 Rocket"
                            
                            if match_type:
                                hunt_results.append({
                                    "代號": t,
                                    "現價": current_price_hunt,
                                    "35Y角度": geo_data_hunt['35Y']['angle'],
                                    "10Y角度": geo_data_hunt['10Y']['angle'],
                                    "3M角度": geo_data_hunt['3M']['angle'],
                                    "G力": geo_data_hunt['acceleration'],
                                    "型態": match_type
                                })
        ...
        if f'hunt_results_{selected_theater}' in st.session_state:
            ...
            if not results_df_hunt.empty:
                st.markdown("---")
                st.markdown("### ⚔️ 戰果清單 (Scan Results)")
                
                # 格式化顯示
                st.dataframe(results_df_hunt.style.format({
                    "現價": "{:.2f}",
                    "35Y角度": "{:.1f}°",
                    "10Y角度": "{:.1f}°",
                    "3M角度": "{:.1f}°",
                ...
                # CSV 下載
                csv_hunt = results_df_hunt.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="📥 下載戰果 (CSV)",
                    data=csv_hunt,
                ...
                selected_target = st.selectbox(
                    "選擇索敵目標",
                    options=target_tickers,
                    key="target_selector_hunter"
                )
                
                if st.button("🔍 鎖定目標並分析", type="primary", use_container_width=True, key="lock_target_btn"):
                    # 載入目標的幾何數據
                    st.session_state.hunt_selected_target = selected_target
                    
                    with st.spinner(f"🔍 正在鎖定 {selected_target}..."):
                        ...
                        if target_geo:
                            ...
                            # 儲存到 session
                            st.session_state.hunt_target_geo = target_geo
                            st.session_state.hunt_target_rating = target_rating
                            
                            st.success(f"✅ 目標已鎖定！信評: **{target_rating[0]} - {target_rating[1]}**")
                        else:
                ...
                if 'hunt_selected_target' in st.session_state:
                    ...
                    target_rating_data = st.session_state.hunt_target_rating
                    
                    st.markdown("---")
                    st.markdown(f"### 📊 目標: {target_ticker}")
                    
                    # 信評卡片
                    st.markdown(f"""
                    <div style='background-color: {target_rating_data[3]}; padding: 15px; border-radius: 10px; text-align: center;'>
                        <h3 style='color: white; margin: 0;'>{target_rating_data[0]} - {target_rating_data[1]}</h3>
                        <p style='color: white; margin: 5px 0;'>{target_rating_data[2]}</p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with col_hunt_left:
                        ...
                        # 統帥筆記
                        st.markdown("**✍️ 統帥筆記**")
                        commander_note_hunt = st.text_area(
                            "自由筆記",
                            height=80,
                            placeholder="補充分析指令...",
                            key="commander_note_hunt"
                        )
                    ...
                    if st.button("🚀 生成索敵戰略提示詞", type="primary", use_container_width=True, key="gen_hunt_prompt_btn"):
                        # 獲取目標價格
                        target_price_hunt = 0.0
                        if target_ticker in st.session_state.get('daily_price_data', {}):
                            df_daily_hunt = st.session_state.daily_price_data[target_ticker]
                            if df_daily_hunt is not None and not df_daily_hunt.empty:
                                target_price_hunt = df_daily_hunt['Close'].iloc[-1]
                        
                        # 生成提示詞
                        council_hunt = TitanAgentCouncil()
                        battle_prompt_hunt = council_hunt.generate_battle_prompt(
                            target_ticker,
                            target_price_hunt,
                            target_geo_data,
                            target_rating_data,
                        ...
                        st.text_area(
                            "📋 複製此提示詞",
                            value=battle_prompt_hunt,
                            height=350,
                            key="prompt_output_hunt"
                        )
                        ...
                        st.download_button(
                            "💾 下載提示詞",
                            battle_prompt_hunt,
                            file_name=f"TITAN_HUNT_{target_ticker}_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
                            mime="text/plain",
                            use_container_width=True
                        )
            else:
