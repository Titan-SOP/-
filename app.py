...
# Create 5 Strategic Tabs for Mobile Optimization
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🛡️ 宏觀大盤",   # Macro Dashboard
    "🏹 獵殺專區",   # Hunter Zone (SOP)
    "🎯 單兵狙擊",   # Sniper HQ (One Window Only)
    "🚀 戰力升級",   # Maintenance & Roadmap
    "📚 戰略百科"    # Knowledge & Tools
])

with tab1:
    with st.expander("1️⃣ 宏觀風控 (Macro Risk)", expanded=True):
        if not df.empty:
    ...
    with st.expander("2️⃣ 高價權值股多空溫度計", expanded=True):
    ...
    with st.expander("4️⃣ PR90 籌碼分佈圖", expanded=True):
        if not df.empty:
            macro_data = get_macro_data(macro, df)
    ...
    with st.expander("6️⃣ 族群熱度雷達 (Sector Heatmap)", expanded=True):
        if not df.empty:
    ...
    with st.expander("1️⃣5️⃣ 成交重心即時預測 (動態 Top 100)", expanded=True):
    ...
    with st.expander("1️⃣6️⃣ 高價權值股趨勢雷達 (Top 50)", expanded=True):
        render_leader_dashboard(
    ...
    with st.expander("1️⃣8️⃣ 台指期月K結算目標價推導 (Settlement Radar)", expanded=True):
